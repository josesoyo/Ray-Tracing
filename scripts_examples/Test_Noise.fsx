...
#r @"../RayTracing/bin/Release/RayTracing.dll"
#r @"../PostProcess/bin/Release/PostProcess.dll"
//Math.Net
...
let Amplitude_big,Amplitude_big2 =  Vector(0.,0.,10.e-6), Vector(0.,10.e-6,0.)
let noise = ([|(10.,Amplitude,0.);(10.,Amplitude2,(PI/7.))|] , t)    // (f,) 
let noise_upconversion = ([|(10.,Amplitude_big,0.);(10.,Amplitude_big2,(PI/6.))|], t)
...
let all (i:int) (path_init:string)=
    ...
    [|1..Nrays|] |> Array.iter(fun x -> //if x%10=0 then printfn "its %i" x
                                        ForwardRay(ray(),objs,mat,0) ) // ok, look like works -   Parallel
    // since there is no noise computation, not it must be computed
    let out_m2 =
        m2.Sensor.SavedData.ToArray() |>
        Array.map(fun content -> let ns = ForwardRay_noise(objs,mat,content.Route,[||])  
                                 //if (Array.isEmpty ns |> not) then printfn "yeag"
                                 //else printfn "NO"
    ...
    printfn "the iteration %d has produced:\nm2:%+A\t\n\n\nm1:%+A" i (out_m2.[10].Noise|> Array.filter(fun x -> x<> 0.)) (m1.Sensor.SavedData.[0].Noise |> Array.filter(fun x -> x<> 0.) ) 
    (*
...
let readPSDs (iters:int) (path_init:string) =
    // Read the files 
    let readOne (i:int):(float[]*float[])*float =
        //let path_save = Path.Combine( path_init,string(i)+"_mod_m1_nup.txt")
        //let path_savef = Path.Combine( path_init,string(i)+"_modf_m1_nup.txt")
        let path_save_2 = Path.Combine( path_init,string(i)+"_mod_m2.txt")
        let path_savef_2 = Path.Combine( path_init,string(i)+"_modf_m2.txt")
    ...
    match iters with
    | x when x > 1 ->
        let mutable  ( f2,  a2), npart0  = readOne 1
        printfn "the number of particles are: %f" npart0
        let freqs = 
            match  f2.Length with
            //| x , _ when x <> 0 -> f1
            | y when y <> 0 -> f2
            | _ -> f2 // very bad if this happens
        [|2..iters|] 
        |> Array.iter(fun x -> 
                                let (_,aa2),npart = readOne x
        ...
        // ( f1,  a1), ( f2,  a2)
        //( freqs,  a1|> Array.map(fun x -> OutTheSquareRoot*sqrt(x/npart0)) ),      
        ( freqs,  a2|> Array.map(fun x -> OutTheSquareRoot*sqrt(x/npart0)) )
                                        
    | _ -> 
       printfn "It was already done since it's a single run!\nBut only a part!"
       let ( f2,  a2), npart0  = readOne 1 //  ( f1,  a1),

       let wavelen =  (match ray().Wavelenght with WaveLength x -> float x)
       let OutTheSquareRoot = (wavelen*wavelen*mat.["Mirror"].LambPPM)/(sqrt(2.**5.)*PI*PI*(float Ltube)*(float Rmirror))
       // mult = 1e-6*1.064e-6*1.064e-6/((2**2.5)*np.pi*np.pi*3e3*0.175)
       
       //( f1,  a1|> Array.map(fun x -> OutTheSquareRoot*sqrt(x/npart0)) ),     
       ( f2,  a2|> Array.map(fun x -> OutTheSquareRoot*sqrt(x/npart0)) )


let  m2 = readPSDs nRepeat path_base
...
let save_freq =
    ...
    | true -> 
        ...
        printfn "this"
        File.WriteAllLines( path_savef, fst m2|> Array.map(fun x -> string(x)) )
